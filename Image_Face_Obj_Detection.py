...
if not os.path.exists(".\\detected_images_"+time.strftime("%d-%m-%Y")):
    os.makedirs(".\\detected_images_"+time.strftime("%d-%m-%Y"))
...
def calling_api(img,kk,ori):
    ...
    if conf> 0.85:
        path = '.\\detected_images_'+time.strftime("%d-%m-%Y")+'\\'+str(str(iters)+'#'+time.strftime("%d-%m-%Y")+' '+texting)
        #print(path)
        path+='.jpg'
